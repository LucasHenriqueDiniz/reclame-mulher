...
export function NewComplaintContent({
  ...
  const buildPayload = useCallback(() => {
    ...
    const attachment_paths: { file_path: string; file_name: string; content_type?: string; size_bytes?: number }[] = [];
  ...
  const getNextLabel = () => {
    if (step === 3) {
      return stepThree.files.length > 0 ? "Próximo passo" : "Continuar sem anexo";
    }
    if (step === TOTAL_STEPS) {
      return "Publicar reclamação";
    }
  ...
  return (
    <ComplaintWizardShell>
      ...
      <Link 
        href="/app/complaints"
        className="fixed top-20 left-4 z-50 group"
      >
        <button className="flex items-center gap-2 px-4 py-2.5 bg-white hover:bg-gray-50 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 border border-gray-200 group-hover:scale-105">
          <svg 
            className="w-5 h-5 text-[#607D8B] group-hover:text-[#1E88E5] transition-colors" 
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
          >
          ...
          <span className="font-['Poppins'] font-medium text-sm text-[#607D8B] group-hover:text-[#1E88E5] transition-colors">
            Voltar
          </span>
        </button>
      </Link>
