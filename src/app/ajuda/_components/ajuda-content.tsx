...
export type TestUserRole = "Pessoa" | "Empresa" | "Admin";

/**
 * One seeded account, as the server hands it over. Deliberately just data: keeping the
 * addresses and the password out of this module is what keeps them out of the client
 * bundle, since a "use client" module is compiled into the route's chunk whether or not
 * the server component above it ever renders.
 */
...
/**
 * Only the presentation lives here. The accounts themselves are server data, passed
 * in as props — see the note on this component's props type.
 */
const ROLE_STYLES: Record<TestUserRole, { icon: LucideIcon; color: string; bg: string }> = {
  Pessoa: { icon: User, color: "text-blue-600", bg: "bg-blue-50" },
  Empresa: { icon: Building2, color: "text-emerald-600", bg: "bg-emerald-50" },
          ...
          <div className="space-y-8">
            ...
            <Card className="border-0 shadow-md">
              ...
              <CardContent className="space-y-3 text-sm text-[#607D8B]">
                <p>
                  Todos os usuários de teste usam a mesma senha: <strong className="text-[#2A3F54]">{sharedPassword}</strong>
                </p>
                <p>
              ...
              <CardContent className="space-y-4">
                {testUsers.map((user) => {
                  ...
                  >
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full ${style.bg}`}>
                      <RoleIcon className={`h-5 w-5 ${style.color}`} />
                    </div>
                ...
                <div className="rounded-lg bg-[#E3F2FD] p-4">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-[#2A3F54]">
                    <code className="rounded bg-white px-2 py-1 text-sm font-mono">{sharedPassword}</code>
                    <CopyButton text={sharedPassword} />
                  </div>
                </div>
              </CardContent>
            </Card>
