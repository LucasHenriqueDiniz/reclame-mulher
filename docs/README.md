...
This folder is laid out as an **Obsidian vault** — open it directly (`File → Open vault → docs/`) and
`[[wiki-links]]` resolve. There is no `.obsidian/` and no `.mcp.json` in this repo yet: the folder
structure is here, the tooling around it is not.
