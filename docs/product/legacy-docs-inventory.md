...
| `TODO.md` | 440 | Operational backlog, P1–P4 | — | `translate` | Half live: the P2/P3/P4 items are real work. **Drop** the "Status base: 91 → 80 tarefas" header and the Sprint-3 completion markers — that is a dated snapshot, and `AUDITORIA_COMPLETA_PROBLEMAS.md` item 8 already says this file is stale |
...
| `RELATORIO_AUDITORIA_FINAL.md` | 270 | Audit snapshot, 2026-07-07, "AUDITORIA COMPLETA" | — | `delete` | Dated snapshot with no open items; the gap list that survives is `AUDITORIA_COMPLETA_PROBLEMAS.md` |
| `RELATORIO_CORRECOES_ACESSIBILIDADE.md` | 225 | Write-up of the accessibility fixes | — | `delete` | Names its own commit, `dfa9baa`. `git show dfa9baa` is the record, and it cannot go stale |
...
| `STATUS_FINAL_PRODUCAO.md` | 261 | Percentage dashboard: "80% PRONTO", "E2E parcial" | — | `delete` | Headline contradicted — the app is deployed and the E2E job is green. ⚠️ It does carry open items: responsiveness is `AUDITORIA_COMPLETA_PROBLEMAS.md` item 3, but **the 4 icon-link aria-labels and the login-form review are only here**. Slice 02 must move those two into the surviving list before deleting this |
...
not, being simply live. The surprises were `AUDITORIA_COMPLETA_PROBLEMAS.md` and
